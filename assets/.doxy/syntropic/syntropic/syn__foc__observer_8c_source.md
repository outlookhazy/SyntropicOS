...
[**File List**](files.md) **>** [**motor**](dir_61470c21c7c2e7c9f977d1e3c2e6c7e0.md) **>** [**syn\_foc\_observer.c**](syn__foc__observer_8c.md)

[Go to the documentation of this file](syn__foc__observer_8c.md)
...
SYN_Status syn_foc_observer_init(SYN_FOCObserver *obs, const SYN_FOCObserverConfig *cfg)
{
    ...
    if (cfg->R <= 0 || cfg->L <= 0 || cfg->G <= 0 || cfg->dt <= 0)
        return SYN_INVALID_PARAM;
...
SYN_Status syn_foc_observer_update(SYN_FOCObserver *obs, q16_t V_alpha, q16_t V_beta, q16_t I_alpha,
                                   q16_t I_beta)
{
    if (obs == NULL)
        return SYN_INVALID_PARAM;
    ...
    int64_t E_sq = ((int64_t)obs->E_alpha_est * obs->E_alpha_est) +
                   ((int64_t)obs->E_beta_est * obs->E_beta_est);
    E_sq >>= Q16_SHIFT;
    ...
    obs->omega_e =
        q16_clamp(obs->pll_i_accum + q16_mul(obs->cfg.Kp_pll, pll_err), -max_speed, max_speed);
