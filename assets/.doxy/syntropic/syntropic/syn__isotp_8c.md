...
|  [**bool**](syn__defs_8h.md#enum-bool) | [**syn\_isotp\_is\_tx\_idle**](#function-syn_isotp_is_tx_idle) (const [**SYN\_ISOTP\_Link**](structSYN__ISOTP__Link.md) \* link) <br>_Check if ISO-TP transmission channel is idle (all frames transmitted)._  |
...
|  void | [**syn\_isotp\_set\_fc\_params**](#function-syn_isotp_set_fc_params) ([**SYN\_ISOTP\_Link**](structSYN__ISOTP__Link.md) \* link, uint8\_t bs, uint8\_t stmin) <br>_Configure Flow Control transmission parameters (BS and STmin)._  |
...
| Type | Name |
| ---: | :--- |
|  uint32\_t | [**syn\_isotp\_decode\_stmin\_us**](#function-syn_isotp_decode_stmin_us) (uint8\_t st\_min) <br>_Decode ISO 15765-2 STmin byte value into microsecond delay._  |
...
### function syn\_isotp\_is\_tx\_idle 

_Check if ISO-TP transmission channel is idle (all frames transmitted)._ 
```C++
bool syn_isotp_is_tx_idle (
    const SYN_ISOTP_Link * link
) 
```
...
true if tx\_state is SYN\_ISOTP\_TX\_IDLE, false if transmission active. 
...
### function syn\_isotp\_set\_fc\_params 

_Configure Flow Control transmission parameters (BS and STmin)._ 
```C++
void syn_isotp_set_fc_params (
    SYN_ISOTP_Link * link,
    uint8_t bs,
    uint8_t stmin
) 
...
* `link` Link handle. 
* `bs` Block size (0 = unlimited, 8 = default 8 frames per block). 
* `stmin` STmin separation time byte value (0 = 0ms). 
...
_Decode ISO 15765-2 STmin byte value into microsecond delay._ 
```C++
