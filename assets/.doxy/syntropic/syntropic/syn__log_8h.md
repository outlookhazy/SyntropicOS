...
| typedef void(\* | [**SYN\_LogOutputFunc**](#typedef-syn_logoutputfunc)  <br>_Log output callback._  |
...
|  void | [**syn\_log\_init**](#function-syn_log_init) ([**SYN\_LogOutputFunc**](syn__log_8h.md#typedef-syn_logoutputfunc) output, [**SYN\_LogLevel**](syn__log_8h.md#enum-syn_loglevel) min\_level) <br>_Initialize the logging system._  |
...
### typedef SYN\_LogOutputFunc 

_Log output callback._ 
```C++
...
The logging system calls this to emit formatted output. The string is NOT null-terminated at position `len` (it is within the buffer, but `len` is the exact number of chars to write).
...
* `output` Function that writes log output (e.g., to UART). 
