...
| Type | Name |
| ---: | :--- |
|  void | [**syn\_metrics\_count**](#function-syn_metrics_count) (const char \* name, int32\_t delta) <br>_Increment/decrement metric counter (no-op when disabled)._  |
...
|  void | [**syn\_metrics\_init**](#function-syn_metrics_init) (void) <br>_Initialize metrics module (no-op when disabled)._  |
|  void | [**syn\_metrics\_record**](#function-syn_metrics_record) (const char \* name, float value) <br>_Record a metric value (no-op when disabled)._  |
...
_Increment/decrement metric counter (no-op when disabled)._ 
```C++
...
_Initialize metrics module (no-op when disabled)._ 
```C++
...
_Record a metric value (no-op when disabled)._ 
```C++
