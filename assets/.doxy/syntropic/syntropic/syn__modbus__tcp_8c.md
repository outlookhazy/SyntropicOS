...
| Type | Name |
| ---: | :--- |
|  uint16\_t | [**get\_response\_pdu\_len**](#function-get_response_pdu_len) (const uint8\_t \* buf) <br>_Calculate expected response PDU byte length for Modbus TCP query._  |
...
_Calculate expected response PDU byte length for Modbus TCP query._ 
```C++
...
* `buf` Pointer to MBAP/PDU payload buffer. 



**Returns:**

Response PDU byte length. 
