...
| Type | Name |
| ---: | :--- |
|  [**bool**](syn__defs_8h.md#enum-bool) | [**get\_field**](#function-get_field) (const char \* sentence, uint8\_t index, char \* dest, size\_t max\_len) <br>_Extract comma-separated NMEA field from sentence._  |
|  uint8\_t | [**hex2dec**](#function-hex2dec) (char c) <br>_Convert hexadecimal character to decimal integer value._  |
|  void | [**parse\_time**](#function-parse_time) (const char \* field, uint8\_t \* h, uint8\_t \* m, uint8\_t \* s, uint16\_t \* ms) <br>_Parse NMEA UTC time field (hhmmss.ss) into components._  |
|  double | [**syn\_atof**](#function-syn_atof) (const char \* s) <br>_Lightweight ASCII string to double conversion for NMEA numerical fields._  |
...
_Extract comma-separated NMEA field from sentence._ 
```C++
...
* `sentence` Input NMEA sentence string. 
* `index` Field index (0-based). 
* `dest` Destination string buffer. 
* `max_len` Maximum capacity of destination buffer. 



**Returns:**

True if field extracted, false otherwise. 
...
_Convert hexadecimal character to decimal integer value._ 
```C++
...
_Parse NMEA UTC time field (hhmmss.ss) into components._ 
```C++
...
* `field` NMEA time field string. 
* `h` Pointer to receive hours (0-23). 
* `m` Pointer to receive minutes (0-59). 
* `s` Pointer to receive seconds (0-59). 
* `ms` Pointer to receive milliseconds (0-999). 
...
_Lightweight ASCII string to double conversion for NMEA numerical fields._ 
```C++
static double syn_atof (
    const char * s
) 
```
...
Converted double value, or 0.0 on NULL/empty input. 
