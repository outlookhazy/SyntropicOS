...
| Type | Name |
| ---: | :--- |
|  void | [**pt\_sem\_signal**](#function-pt_sem_signal) ([**SYN\_PT\_Sem**](structSYN__PT__Sem.md) \* sem) <br>_Increment the semaphore count, unblocking a waiting protothread._  |
...
| Type | Name |
| ---: | :--- |
| define  | [**PT\_SEM\_BLOCK**](syn__pt__sem_8h.md#define-pt_sem_block) (pt, task, sem) `/* multi line expression */`<br>_Block task execution (SYN\_TASK\_BLOCKED) until semaphore count is &gt; 0, then decrement it._  |
...
| define  | [**PT\_SEM\_SIGNAL**](syn__pt__sem_8h.md#define-pt_sem_signal) (sem) `[**pt\_sem\_signal**](syn__pt__sem_8h.md#function-pt_sem_signal)(sem)`<br>_Macro wrapper for pt\_sem\_signal._  |
...
Safe to call from ISR context. The increment runs inside a critical section: `count++` on a volatile int16\_t is a load-modify-store, and a preempting ISR signaling the same semaphore would otherwise be overwritten by the stale store (lost signal).
...
### define PT\_SEM\_BLOCK 

_Block task execution (SYN\_TASK\_BLOCKED) until semaphore count is &gt; 0, then decrement it._ 
```C++
#define PT_SEM_BLOCK (
    pt,
    task,
...
_Macro wrapper for pt\_sem\_signal._ 
```C++
