...
|  void | [**syn\_task\_boost\_priority**](#function-syn_task_boost_priority) ([**SYN\_Task**](structSYN__Task.md) \* task, uint8\_t temp\_prio) <br>_Temporarily boost a task's execution priority._  |
...
|  void | [**syn\_task\_restore\_priority**](#function-syn_task_restore_priority) ([**SYN\_Task**](structSYN__Task.md) \* task) <br>_Restore a task's priority back to its configured base priority._  |
...
|  void | [**syn\_task\_set\_base\_priority**](#function-syn_task_set_base_priority) ([**SYN\_Task**](structSYN__Task.md) \* task, uint8\_t new\_prio) <br>_Dynamically set a task's base priority and active priority._  |
...
| Type | Name |
| ---: | :--- |
| define  | [**SYN\_SCHED\_PRIO\_LEVELS**](syn__sched_8h.md#define-syn_sched_prio_levels)  `8`<br>_Maximum number of priority levels (0 to N-1). Override in syn\_config.h if more are needed._  |
...
### function syn\_task\_boost\_priority 

_Temporarily boost a task's execution priority._ 
```C++
void syn_task_boost_priority (
    SYN_Task * task,
    uint8_t temp_prio
) 
```
...
Clamps temp\_prio &lt;= base\_priority (0 = highest priority). Will not demote a task to a lower priority than its base priority.
...
* `task` Task to boost. 
* `temp_prio` Temporary elevated priority. 
...
### function syn\_task\_restore\_priority 

_Restore a task's priority back to its configured base priority._ 
```C++
void syn_task_restore_priority (
    SYN_Task * task
) 
```
...
### function syn\_task\_set\_base\_priority 

_Dynamically set a task's base priority and active priority._ 
```C++
void syn_task_set_base_priority (
    SYN_Task * task,
    uint8_t new_prio
) 
```
...
### define SYN\_SCHED\_PRIO\_LEVELS 

_Maximum number of priority levels (0 to N-1). Override in syn\_config.h if more are needed._ 
```C++
