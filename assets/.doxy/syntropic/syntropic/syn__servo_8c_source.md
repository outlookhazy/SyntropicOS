...
[**File List**](files.md) **>** [**motor**](dir_61470c21c7c2e7c9f977d1e3c2e6c7e0.md) **>** [**syn\_servo.c**](syn__servo_8c.md)

[Go to the documentation of this file](syn__servo_8c.md)
...
static uint16_t angle_to_us(const SYN_Servo *s, uint16_t angle)
{
    ...
    return (uint16_t)(s->pulse_min + (range_us * angle) / s->angle_range);
}

static uint16_t clamp_us(const SYN_Servo *s, uint16_t us)
{
    if (s == NULL)
        return us; /* LCOV_EXCL_LINE: Defensive NULL check or invalid parameter fallback */
    if (us < s->pulse_min)
        return s->pulse_min;
    if (us > s->pulse_max)
        return s->pulse_max;
