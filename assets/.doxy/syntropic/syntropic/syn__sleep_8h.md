...
Safe to call from ISR context. The \|= on the volatile mask is a load-modify-store, so without a critical section a preempting ISR's lock update would be overwritten by the stale store (lost lock — the CPU could then sleep through an active transfer).
...
Safe to call from ISR context — see [**syn\_sleep\_lock()**](syn__sleep_8h.md#function-syn_sleep_lock).
