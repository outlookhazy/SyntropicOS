...
[**File List**](files.md) **>** [**debug**](dir_6f573363694cd388330fe66cf20dcff2.md) **>** [**syn\_trace.c**](syn__trace_8c.md)

[Go to the documentation of this file](syn__trace_8c.md)
...
bool syn_trace_read(const SYN_Trace *trace, uint32_t index, SYN_TraceEntry *entry)
{
    ...
    uint32_t available = (trace->count < trace->capacity) ? trace->count : trace->capacity;
    if (index >= available)
        return false;
...
void syn_trace_dump(const SYN_Trace *trace, SYN_TracePrintFunc print)
{
    ...
    for (uint32_t i = 0; i < available; i++) {
        if (syn_trace_read(trace, i, &entry)) {
            ...
            /* evt=0xXXXX */
            memcpy(buf + pos, "evt=0x", 6);
            pos += 6;
            syn_fmt_hex(num, sizeof(num), entry.event_id, 4);
            memcpy(buf + pos, num, 4);
            pos += 4;

            /* val=0xXXXX */
            memcpy(buf + pos, " val=0x", 7);
            pos += 7;
