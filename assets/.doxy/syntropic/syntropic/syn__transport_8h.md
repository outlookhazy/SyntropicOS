...
| Type | Name |
| ---: | :--- |
|  [**bool**](syn__defs_8h.md#enum-bool) | [**syn\_transport\_has\_data**](#function-syn_transport_has_data) (const [**SYN\_Transport**](structSYN__Transport.md) \* t) <br>_Check if transport has unread data available (non-blocking)._  |
...
### function syn\_transport\_has\_data 

_Check if transport has unread data available (non-blocking)._ 
```C++
static inline bool syn_transport_has_data (
    const SYN_Transport * t
) 
```
...
true if data is ready or readiness check is unhandled. 
